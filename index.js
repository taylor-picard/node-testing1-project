...
function trimProperties(obj) {
  let newObj = {};
  Object.keys(obj).forEach(key=> newObj[key] = obj[key].trim());
  return newObj;
}
...
class Seasons {
   ...
   */
  next() {
    const newSeason = this.seasons[this.current]
    if(this.current < 3){
      this.current++
    }else{
      this.current = 0
...
class Car {
  ...
  constructor(name, tankSize, mpg) {
    ...
    this.milesTilEmpty = this.tank * mpg
    this.mpg = mpg
  }
   ...
   */
  drive(distance) {
    if(distance <= this.milesTilEmpty){
  ...
  refuel(gallons) {
    if(gallons <= this.tankSize-this.tank){
      ...
      this.milesTilEmpty = this.tank * this.mpg
    }else{
      this.tank = this.tankSize
      this.milesTilEmpty = this.tank * this.mpg
    }
    return this.milesTilEmpty
  }
}
